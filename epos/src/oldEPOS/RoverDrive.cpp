...
bool SetVelocity(HANDLE p_DeviceHandle, unsigned short p_usNodeId, unsigned int & p_rlErrorCode, long Velocity)
{
	int lResult = MMC_SUCCESS;
	stringstream msg;
    long targetvelocity = Velocity;
...
bool HaltVelocity(HANDLE p_DeviceHandle, unsigned short p_usNodeId, unsigned int & p_rlErrorCode)
{
    int lResult = MMC_SUCCESS;
...
int DisableMotor(unsigned int* p_pErrorCode, unsigned short int nodeId)
{
    int lResult = MMC_SUCCESS;
    unsigned int lErrorCode = 0;
	for (int deviceNum = 1; deviceNum < (NumDevices + 1); deviceNum++)
...
int DisableDemo(unsigned int* p_pErrorCode)
{
	int lResult = MMC_SUCCESS;
	for (int deviceNum = 1; deviceNum < (NumDevices + 1); deviceNum++)
	{
